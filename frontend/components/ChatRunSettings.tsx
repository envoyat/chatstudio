...
export default function ChatRunSettings({ className }: ChatRunSettingsProps) {
  ...
  return (
    <div className={`fixed right-0 top-0 z-50 h-full ${className}`}>
      {/* Toggle Button */}
      <Button
        onClick={togglePanel}
        variant="outline"
        ...
        } z-10 bg-background/95 backdrop-blur-sm border shadow-md hover:shadow-lg`}
      >
        {isOpen ? (
      ...
      {/* Side Panel */}
      <div
        className={`h-full bg-background/95 backdrop-blur-sm border-l shadow-xl transition-transform duration-300 ease-in-out ${
